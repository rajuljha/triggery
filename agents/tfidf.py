...
from numpy import dot, sum, unique
...
class TFIDF(AtarashiAgent):
    ...
    def __tfidfsumscore(self, inputFile):
        ...
        for counter, value in enumerate(
            sklearn_representation[: len(sklearn_representation) - 1], start=0
        ):
            sim_score = sum(value)
            score_arr.append(
                {
...
if __name__ == "__main__":
    ...
    if tfidf_similarity == "CosineSim":
        ...
        print(
            "License Detected using TF-IDF algorithm + cosine similarity "
            + str(scanner.scan(filename))
        )
    else:
        scanner.setSimAlgo(TFIDF.TfidfAlgo.scoreSim)
        print(
            "License Detected using TF-IDF algorithm + sum score "
            + str(scanner.scan(filename))
        )
