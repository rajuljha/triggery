...
class KeywordAgent(AtarashiAgent):
    ...
    _keyword_file_path = os.path.join(
        os.path.dirname(__file__), "..", "data", "license_keywords.txt"
    )
    _refs_file_path = os.path.join(
        os.path.dirname(__file__), "..", "data", "licenses", "license_refs_combined.csv"
    )
    ...
    def __init__(self, licenseList=None, verbose=0):
        ...
        self.license_shortnames_and_refs = self.load_license_shortnames_and_refs(
            self._refs_file_path
        )
    ...
    def load_license_shortnames_and_refs(self, file_path):
        ...
        try:
            ...
            for keyword in df["key"].dropna():
                patterns.append(
                    re.compile(r"\b" + re.escape(str(keyword)) + r"\b", re.IGNORECASE)
                )
        except Exception as e:
            if self.verbose > 0:
                print(f"Failed to load keywords from {file_path}: {e}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Scan a file or directory for license keywords."
    )
